...
// Add prop validation for DynamicMarkers
DynamicMarkers.propTypes = {
  providerPosition: PropTypes.object,
    ...
    const getLocationName = async () => {
      try {
        ...
        if (!currentPosition || !currentPosition.lat || !currentPosition.lng) {
          ...
          return "Unknown location"; // Return a default value
        }
        ...
        if (!response.ok) {
          // If API call fails, log the error and fall back to coordinates
          console.error(`Failed to fetch location name: ${response.status}`);
          const fallbackName = `${currentPosition.lat.toFixed(4)}, ${currentPosition.lng.toFixed(4)}`;
        ...
        return displayName; // Return the fetched or fallback name
      } catch (error) {
        console.error("Error getting location name:", error);

        // Fallback to coordinates if an exception occurs
  ...
  useEffect(() => {
    ...
    const handleLocationUpdate = (data) => {
      try {
        ...
        // Update current position for service provider
    ...
    // Start watching position for service provider
        ...
        <div className="flex flex-col h-full">
          ...
          {bookingState === "reviewing" && currentRequest && (
            <div className="absolute inset-0 bg-white bg-opacity-90 flex flex-col items-center justify-center p-4 pointer-events-auto">
              <div className="w-full max-w-sm bg-white rounded-lg shadow-lg p-4 text-sm">
                <h3 className="text-lg font-bold mb-3">Service Request</h3>

                {/* Location information at top of booking dialog */}
                <div className="bg-blue-50 p-2 rounded-lg mb-3 text-xs">
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <p className="text-xs text-gray-600">User Location:</p>
                      <p className="text-sm font-medium">
                        {currentRequest?.details?.userLocationName || currentRequest?.userLocationName || "Location unavailable"}
                      </p>
                    </div>
                  </div>
                </div>
