...
const SignupServiceProvider = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // File uploads are not strictly required for the initial submission based on backend logic,
      // but keeping the checks here if frontend validation is desired.
      ...
      if (error.response && error.response.data && error.response.data.error) {
        toast.error(error.response.data.error); // Display specific backend error
      } else if (error.message) {
         toast.error(error.message); // Display generic error message
      } else {
        toast.error("An unexpected error occurred during registration."); // Fallback generic error
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-sky-500 to-blue-600 py-8 px-4 sm:px-6 lg:px-8 overflow-y-auto">
      ...
      <div className="fixed top-0 left-0 w-[60%] h-full bg-blue-300 opacity-30 rounded-full transform -translate-x-1/4 -z-10"></div>
      <div className="fixed bottom-0 right-0 w-[60%] h-full bg-blue-300 opacity-30 rounded-full transform translate-x-1/4 -z-10"></div>
